...
$sex = $_POST["gender"];
$dob = $_POST["birthDate"];
$ids = $_FILES['idCard']['tmp_name'];
$idCard = addslashes(file_get_contents($ids));
...
$pwd = $_POST["password"];
$pps = $_FILES['profileImage']['tmp_name'];
$pp = addslashes(file_get_contents($pps));
    ...
    if($getId){
        ...
        if ($userContact){
            ...
            if($login){
                ...
                if($userAccount){
                    header("location:login.php?status=register");
                }
            }
        } else {
